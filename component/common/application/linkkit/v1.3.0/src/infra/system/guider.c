...
#ifdef MQTT_DIRECT
      printf("host:%s",domain_mqtt_direct[iotx_guider_region]);
...
int iotx_guider_authenticate(iotx_conn_info_t *conn)
{
    ...
    _fill_conn_string(conn->host_name, len+1,
                      "%s.%s",
                      dev.product_key,
    ...
    _fill_conn_string(conn->username, len+1,
                      "%s&%s",
                      dev.device_name,
                      dev.product_key);
	sys_debug("================");
	sys_debug("product key: %s",dev.product_key);
	sys_debug("================");
	sys_debug("================");
	sys_debug("username: %s",conn->username);
	sys_debug("================");
    ...
    _fill_conn_string(conn->password, len+1,
                      "%s",
                      guider_sign);
#else   /* MQTT_DIRECT */

...
static void usbd_verify_int_out_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	switch (status) {
		case USB_ESUCCESS:
			...
			rtw_memset(vdev->int_out_buf, 0, req->actual);
			ret = usb_ep_queue(ep, req, 1);
			if (ret){
...
/*
	...
	USBD   full/high   iso  in/out  buf_size(<=512)  long_run_flag
	USBD   full/high   int  in/out  transfer_size  long_run_flag
*/
u32
CmdUsbdTest(
    IN  u16 argc,
...
)
{
	...
	// macro USBD_ISO_VERIFY should set to 0
		...
		if(strcmp(argv[2], "in") == 0) {
			...
			vdev->alt_cb = UsbdVerifyIntIn;
		} else {
			DBG_8195A("usb device interrupt out transfer test\n");
			vdev->alt_cb = UsbdVerifyIntOut;
		}
	} else {

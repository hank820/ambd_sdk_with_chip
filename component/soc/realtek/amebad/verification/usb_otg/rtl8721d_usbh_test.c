...
void UsbhVerifyBulk(struct usb_device *udev)
{
	...
	while(1) {
		...
		//bulk out
		usb_bulk_msg(udev, usb_sndbulkpipe(udev, usbh_test_dev->out_bulk_addr), bufout, usbh_test_dev->bulk_buf_size, &actual_length, 1000);
		//bulk in
		usb_bulk_msg(udev, usb_rcvbulkpipe(udev, usbh_test_dev->in_bulk_addr), bufin, usbh_test_dev->bulk_buf_size, &actual_length1, 1000);
...
/*
	...
	USBH   iso  in/out  packet_number  packet_size(<=512)  long_run_flag
	USBH   int  in/out  transfer_size  long_run_flag
*/
u32
CmdUsbhTest(
    IN  u16 argc,
